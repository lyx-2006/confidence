...
from .config import (
    FLOAT_TOLERANCE, HISTORICAL_CAPTURE, HISTORICAL_CONSTRUCTION,
    HISTORICAL_TEST, INFERENCE_PATH, LAYERS, MODEL_PATH, POSITIONS,
    RESULTS_ROOT, SMOKE_LAYERS,
)
from .fingerprint import check_or_write, experiment_config
from .io_utils import append_jsonl, array_hash, atomic_json, atomic_jsonl, atomic_npz, atomic_text, canonical_hash, load_jsonl, sha256_file
...
def capture_hidden(root: Path, candidates: Sequence[dict[str, Any]], *, smoke: bool, resume: bool, config: dict[str, Any]) -> dict[str, Any]:
    ...
    if set(existing) == expected:
        summary = {"status": "complete", "smoke_only": smoke, "completed": len(existing), "expected": len(expected), "reused_hidden": 0, "new_gpu_forwards": 0, "resumed_noop": True}
        atomic_json(root / "progress" / "capture_progress.json", summary); return summary
    ...
    if not missing:
        summary = {"status": "complete", "smoke_only": smoke, "completed": len(existing), "expected": len(expected), "reused_hidden": 0, "new_gpu_forwards": 0, "resumed_noop": False}
        atomic_json(root / "progress" / "capture_progress.json", summary); return summary
    ...
    for candidate in missing:
        ...
        try:
            ...
            lat = int(located["P1_LAT"]["processed_index"]); panl = int(located["P1_PANL"]["processed_index"]); sac = int(located["P1_SAC"]["processed_index"])
            if not (lat < panl < sac) or lat == panl or "\n" not in str(located["P1_PANL"]["token_text"]): raise ValueError("LAT/PANL position gate failed")
            answer_span = list(located["phase1_answer_span"])
            if lat != int(answer_span[1]) - 1: raise ValueError("LAT is not the final true answer token")
            forward = run_hooked_forward(inference.model, inputs, modules, {name: int(located[name]["processed_index"]) for name in POSITIONS}, logits_positions=[sac]); new_forwards += 1
            ...
            expected_keys = {f"{position}__L{layer}" for position in POSITIONS for layer in layers}
            if set(arrays) != expected_keys: raise ValueError("Joint hidden capture is incomplete")
            hidden_metadata = {key: {"dtype": str(value.dtype), "shape": list(value.shape), "sha256": array_hash(value)} for key, value in arrays.items()}
            relative = Path("artifacts") / "hidden" / f"{case_id}.npz"; destination = root / relative; atomic_npz(destination, arrays)
            result = {"status": "completed", "case_id": case_id, "family_id": candidate["family_id"], "item_id": str(candidate["item_id"]), "canonical_answer": candidate["canonical_answer"], "sa_side": candidate["sa_side"], "phase0_raw_answer": candidate["phase0_raw_answer"], "phase1_prompt_hash": candidate["phase1_prompt_hash"], "rendered_prompt_hash": canonical_hash(rendered), "phase1_answer_span": answer_span, "phase1_answer_token_ids": located["phase1_answer_token_ids"], "positions": located, **score, "hidden_file": str(relative), "hidden_sha256": sha256_file(destination), "hidden_keys": sorted(arrays), "hidden_dtype": "float16", "hidden_metadata": hidden_metadata, "source": "new_joint_clean_forward", "parity": parity, "config_fingerprint": config["fingerprint"]}
            append_jsonl(clean_path, result); existing[case_id] = result
        except Exception as exc:
            failure = {"stage": "prepare_capture", "case_id": case_id, "type": type(exc).__name__, "message": str(exc), "timestamp": time.time()}; append_jsonl(root / "progress" / "failures.jsonl", failure); atomic_json(root / "progress" / "capture_progress.json", {"status": "failed", "completed": len(existing), "expected": len(expected), "failure": failure}); raise
        atomic_json(root / "progress" / "capture_progress.json", {"status": "running", "completed": len(existing), "expected": len(expected), "reused_hidden": 0, "new_gpu_forwards": new_forwards, "elapsed_seconds": time.time() - started, "last_case_id": case_id})
    summary = {"status": "complete", "smoke_only": smoke, "completed": len(existing), "expected": len(expected), "reused_hidden": 0, "new_gpu_forwards": new_forwards, "resumed_noop": new_forwards == 0, "elapsed_seconds": time.time() - started}
    atomic_json(root / "progress" / "capture_progress.json", summary); return summary
