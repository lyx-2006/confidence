...
from .config import DEFAULT_ALPHAS, DEFAULT_STEERING_LAYERS, RESULTS_ROOT, SMOKE_ROOT, TEMPLATE_NAMES, validation_root
...
def run_pipeline(*,templates:Sequence[str],num_gpus:int,resume:bool,smoke:bool,stages:Sequence[str],steering_layers:Sequence[int],alphas:Sequence[float],validation_cases:int|None=None)->dict[str,Any]:
    ...
    root=SMOKE_ROOT if smoke else (validation_root(validation_cases) if validation_cases else RESULTS_ROOT)
